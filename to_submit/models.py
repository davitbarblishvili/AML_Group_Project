...
results = pd.DataFrame(
    columns=['Model', 'Accuracy', 'F1-Score', 'Fit-Time', 'Predict-Time'])


# RUN ALL MODELS ON ALL DATA
# Logistic Regression
results = results.append(
    lr(train_x1, train_y1, test_x1, test_y1), ignore_index=True)
results = results.append(
    lr(train_x2, train_y2, test_x2, test_y2), ignore_index=True)
results = results.append(
    lr(train_x3, train_y3, test_x3, test_y3), ignore_index=True)
# SVM
results = results.append(
    run_SVM(train_x1, train_y1, test_x1, test_y1), ignore_index=True)
results = results.append(
    run_SVM(train_x2, train_y2, test_x2, test_y2), ignore_index=True)
results = results.append(
    run_SVM(train_x3, train_y3, test_x3, test_y3), ignore_index=True)
# KNN (with best parameters)
results = results.append(
    run_knn(20, 2, train_x1, train_y1, test_x1, test_y1), ignore_index=True)
results = results.append(
    run_knn(20, 2, train_x2, train_y2, test_x2, test_y2), ignore_index=True)
results = results.append(
    run_knn(20, 2, train_x3, train_y3, test_x3, test_y3), ignore_index=True)
# Naive Bayes
results = results.append(
    run_GNB(train_x1, train_y1, test_x1, test_y1), ignore_index=True)
results = results.append(
    run_GNB(train_x2, train_y2, test_x2, test_y2), ignore_index=True)
results = results.append(
    run_GNB(train_x3, train_y3, test_x3, test_y3), ignore_index=True)
# Random Forest (with best resampling)
results = results.append(run_random_forest_smote(
    train_x1, train_y1, test_x1, test_y1), ignore_index=True)
results = results.append(run_random_forest_smote(
    train_x2, train_y2, test_x2, test_y2), ignore_index=True)
results = results.append(run_random_forest_smote(
    train_x3, train_y3, test_x3, test_y3), ignore_index=True)
# CNN
results = results.append(
    cnn(train_x1, train_y1, test_x1, test_y1), ignore_index=True)
results = results.append(
    cnn(train_x2, train_y2, test_x2, test_y2), ignore_index=True)
results = results.append(
    cnn(train_x3, train_y3, test_x3, test_y3), ignore_index=True)
